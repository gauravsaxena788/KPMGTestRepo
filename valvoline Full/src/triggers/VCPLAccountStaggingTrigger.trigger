trigger VCPLAccountStaggingTrigger on VCPL_Account_Stagging__c (before insert,after insert) {

    try{
        VCPL_AccountStaggingHandler invTriggerHandler = new VCPL_AccountStaggingHandler();
        if(Trigger.isBefore && Trigger.IsInsert){
            Map<String, Schema.SObjectField> mapSchemaWithFieldNew = Schema.SObjectType.VCPL_Account_Stagging__c.fields.getMap();
            Schema.SObjectType mapSchemaWithField = Schema.getGlobalDescribe().get('VCPL_Account_Stagging__c');
        ...
        if(Trigger.isAfter && Trigger.isInsert){
            invTriggerHandler.InsertAccount(trigger.new);
        }
    }
    catch(exception e){
