({
    ...
    assignToTBMJS : function(component, event, helper) {   
        var strCaseId =component.get("v.recordId");    
        if(strCaseId)
        {
            var action = component.get("c.assignToTBM");
            action.setParams({ strRecordId : strCaseId})
            action.setCallback(this, function(response){
                var state = response.getState();
                if (state === "SUCCESS")
                {
                    helper.ShowToast(component, event,'dismissible','Case Assigned to TBM','Success!','success',20000);
