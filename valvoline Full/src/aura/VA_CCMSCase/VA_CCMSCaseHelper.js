({
    getCases: function(component,event,recId) {
        ...
        action.setParams({ "Caseid" : recId });CompaintPicklistchange
    ...
    getAccountName : function(component,event) {
        var recordIdM = component.get("v.recordId");
        var recordIdC = component.get("v.passedAccId");
        var recordId = "";
        //For Communities(Used for tab switching in child Acc)
        if(! $A.util.isUndefinedOrNull(recordIdC) || recordIdC ===""){
            recordId = recordIdC;
        }else{
            recordId = recordIdM;
        }
        console.log('FFFFF recordId-->'+recordId);
        ...
        console.log('FFFFF val before APEX call-->'+JSON.stringify(action));
        action.setCallback(this, function(actionresult) {
            
            var state = actionresult.getState();
            if(state === "SUCCESS"){
                var result =actionresult.getReturnValue();
                console.log('FFFFF from APEX-->'+JSON.stringify(result));
    ...
    createCase : function(component, event) {
        ...
        action.setCallback(this, function(actionresult) {
            ...
            if(state === "SUCCESS")
            {
                ...
                var getCompContext = component.get("v.compContext");
                if(! $A.util.isUndefinedOrNull(getCompContext) && getCompContext ==='child' ){
                	var urlEvent = $A.get("e.force:navigateToURL");
                	urlEvent.setParams({
                   		// "url": '/case/'+recId
                  		//"url" : '/case-detail?id='+result
                  		"url" : '/case/'+result+'/detail'
                	});
                 urlEvent.fire();    
